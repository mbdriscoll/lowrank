...
#define IDX_IMG(t0,p,t1,x,y,z) ((t0)*P*T1*b*b*b + (p)*T1*b*b*b + (t1)*b*b*b + (x)*b*b + (y)*b + (z))

void svthresh(float thresh, int b, // threshold and block size
    int sx, int sy, int sz,        // shift values
    int T0, int P, int T1,         // time/space/time dimensions
    ...
    #pragma omp parallel
    {
        ...
        for (int z = -sz; z < Z; z += b) {
            ...
            for (int bz = 0; bz < b; bz++) { if (0 <= z+bz && z+bz < Z) {
                  imgs[ IDX_IMG(t0,p,t1,x+bx,y+by,z+bz) ] = block[ IDX_BLK(t0,t1,bx,by,bz) ];
            }}}}}}}}
            ...
            for (int by = 0; by < b; by++) { if (0 <= y+by && y+by < Y) {
            for (int bz = 0; bz < b; bz++) { if (0 <= z+bz && z+bz < Z) {
                  block[ IDX_BLK(t0,t1,bx,by,bz) ] = imgs[ IDX_IMG(t0,p,t1,x+bx,y+by,z+bz) ];
            }}}}}}}}
